## This codes work in EdgeR version 3.8.5.

library(edgeR)
setwd("C://Work/PTMC/20150112/(2) BRAF/")
targets <- readTargets("targets (2-1).txt")
head(targets)
x <- read.delim("(2-1) BRAF_MT_PTMC(15)PTC(219).txt")
dim(x)

y <- DGEList(count=x[,2:235], group=targets$Size, genes=x[,1])
y
...
plotMDS(y, cex=0.5, col=c(rep("red",15),rep("blue",219)), method="bcv")
plotBCV(y)
...
write.csv(Top, "G://Research//014 MicroPTC//Results/(4) Node meta/(4-2) DEG result.csv")

# heatmap

Top <- topTags(et, 64)
y <- DGEList(count=x[,2:178], group=targets$Size, genes=x[,1])
...
ptcol <- c(rep("red",16), rep("skyblue",161))
...
topGO(go,ont="BP",sort="Down",n=30)
write.csv(go, "G://Research//014 MicroPTC//Results/(2) BRAF/(2-1) GO result.csv")
